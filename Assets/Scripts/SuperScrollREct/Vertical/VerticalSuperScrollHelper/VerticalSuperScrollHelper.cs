...
{
    public TestVerticalSuperScroll testVerticalSuperScroll;
    public void DetectValueChange_float(ref float bufferValue,ref float originalValue,ref bool hasChange)
    {
        if(originalValue!=bufferValue)
        {
            hasChange = true;
            originalValue = bufferValue;
        }
    }
    public void InitialIndexBufferValue(ref float bufferValue, ref float originalValue)
    {
        bufferValue = originalValue;
    }
    ...
    public void SetContentWidth(RectTransform scrollRectTran, RectTransform contentRectTran, float itemNum, float maxItemNum, float queue, float itemWidth, float itemHeight,float viewYNum)  //左對齊
    {
        Debug.Log("Set");
        ...
        //float sizeY = Mathf.Ceil(maxItemNum / queue) * itemHeight;      
        float sizeY = viewYNum * itemHeight;      
        ...
        RectTransformUtil.Instance.SetAnchor(contentRectTran, 0, 1, 1, 1);
    }

    public void SetContentYPos(RectTransform contentRectTran,float yPos)
    {
        RectTransformUtil.Instance.SetPosY(contentRectTran, yPos);
    }

    public void SetScrollViewPos(RectTransform scrollRectTran,RectTransform positionRectTran)
    {
        RectTransformUtil.Instance.SetPosEqualOtherTran(scrollRectTran, positionRectTran);
    }

    public void InitialInsCountitemWidth(GameObject instanPrefab, RectTransform contentRectTran, List<GameObject> itemList, List<int> dataList, float itemNum, float maxItemNum, float queue, float containRow, float viewYNum, float itemWidth, float itemHeight, ref int lastIndex)
    {
    ...
    public void ItemDestroy(List<GameObject> itemList)
    {
        for (int i = 0; i < itemList.Count; i++)
        {
            GameObject.Destroy(itemList[i]);
    ...
    public void InsCountitemWidth(GameObject instanPrefab, RectTransform contentRectTran, List<GameObject> itemList, List<int> dataList, float itemNum, float maxItemNum, float queue,float containRow, float viewYNum, float itemWidth, float itemHeight, ref int lastIndex)
    {
    ...
    public void LockScrollViewYMax(RectTransform contentTran, float maxX)
    {
        RectTransformUtil.Instance.YPosFloorLock(contentTran, maxX);
    }
    public void LockScrollViewYMin(RectTransform contentTran, float minX)
    {
        RectTransformUtil.Instance.YPosCeilingLock(contentTran, minX);
    }
    ...
    public void OnScrollMoveWidth(RectTransform scrollRectTran, RectTransform contentRectTran, List<GameObject> itemList, List<int> dataList, float itemNum, float maxItemNum, float queue,float containRow, float itemWidth, float itemHeight, ref int firstIndex, ref int lastIndex, ref int nowIndex, ref float xPositivePosBuffer, ref float xNegativePosBuffer)
    {
        ...
        while (Mathf.Abs(contentRectTran.anchoredPosition.y) - itemHeight * (containRow-1) > itemHeight * (firstIndex / queue) && nowIndex >= firstIndex && (nowIndex + (int)itemNum) < (maxItemNum))
        {
            ...
            for (int i = 0; i < queue; i++)
            {
                ...
                {
                    if (_first.activeInHierarchy == false)
                    {
        ...
        while (Mathf.Abs(contentRectTran.anchoredPosition.y) - itemHeight * (containRow-1) < itemHeight * firstIndex / queue && firstIndex <= nowIndex &&firstIndex!=0)
        {
            ...
            for (int i = 0; i < queue; i++)
            {
                ...
                {
                    if(_last.activeInHierarchy==false)
                    {
    ...
    public void SetIndex(ref int index,int value)
    {
        index = value;
    }
